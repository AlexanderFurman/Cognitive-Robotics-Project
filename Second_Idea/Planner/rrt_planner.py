...
class RRTPlanner:
    ...
    def sample_random_state(self):
        node_in_tree = True
        while node_in_tree:
            rand_angles = np.random.uniform(-self.environment.robot.min_joint_limit, -self.environment.robot.max_joint_limit, 1, self.environment.robot.n_dof) # check if uniform works, try also np.random.rand
    ...
    def extend(self, rand_node, near_node):
        ...
        new_node_vector = np.norm(rand_node_vector - near_node_vector)*self.step
    ...
    def rrt(self, initial_joint_values):
        ...
        for _ in range(self.max_iterations):
            ...
            nearest_node = self.tree.nearest_node(random_node)
            ...
            if not self.query_state_cause_collision(new_node.vectorized_values()) and not self.tree.query_node_in_graph(new_node):
                ...
                self.tree.add_edge(nearest_node, new_node) # check that predecessor, successor working
